...
void CInputExtension::SetActionListener() const
{
	...
	if (pActionMapMan)
	{
		...
		const XmlNodeRef rootNode = GetISystem()->LoadXmlFromFile("libs/config/defaultProfile.xml");
		if (rootNode)
		{
			pActionMapMan->Clear();
		...
		pActionMapMan->EnableFilter(0, false);

		IActionMap* pActionMap = pActionMapMan->GetActionMap("player");
		if (pActionMap)
		{
