...
class UiCategory_P4(UiCategory_Base):
	def __init__(self, category_name, parent):
		...
		self.options_list.remove('third_party_rootfolder')
				
		# Create base widget
		self.add_option_widget('auto_run_bootstrap')
	...
	def get_group_box_name(self, option_name):
		if option_name == 'auto_run_bootstrap':
			return "State"		
	...
	def resolve_option_dependencies(self, category_name, option_name, is_valid, value):
		...
		# Handle P4 User root folder usage
		if option_name == 'third_party_p4host':
			self.remove_option_widget('third_party_rootfolder')
			if not value:
				add_to_group_box =  self.get_group_box_name('p4client')
				self.add_option_widget('third_party_rootfolder', add_to_group_box)
		...
		# Do not show other options if checkbox is checked  as 'False' is a valid return value
		if option_name == 'auto_run_bootstrap' and is_valid:
			is_valid = (value == 'True')
		...
		# Handle 3rd Party root folder usage
		if option_name == 'third_party_p4client':
			if is_valid:
				add_to_group_box =  self.get_group_box_name('third_party_p4client')
				self.add_option_widget('third_party_rootfolder', add_to_group_box)
			else:
				self.remove_option_widget('third_party_rootfolder')
		...
		if is_valid:
			...
			for option_to_delete in self.options_list[element_index+1:]:
				...
				if self.options_widgets.get(option_to_delete, None):
					...
					if 'p4client' in option_to_delete:
						self.remove_option_widget('third_party_rootfolder')

	def validate_category(self):
		bootstrap_widget = self.options_widgets.get('auto_run_bootstrap', None)
		if not bootstrap_widget or bootstrap_widget.get_value() == "False":
			return (True, "")		
		
		third_party_root_folder_widget =  self.options_widgets.get('third_party_rootfolder', None)		
		if not third_party_root_folder_widget or not third_party_root_folder_widget.revalidate():
			return (False, "[ERROR] - Category '%s' is incomplete. Please populate form until 'third_party_rootfolder' has been set or disable bootstrap." % self.category_name)
...
class GuiTask_ModifyWafConfig(IGuiTask):
	...
	def read_waf_option_config(self):		
		...
		for section in self.waf_ctx.default_settings:
			if section == "Bootstrap Support":
				self.categories[section] =  UiCategory_P4(section, self)
			else:
