...
Boar_x =
{
	...
	AnimDatabase3P = "Animations/Mannequin/ADB/wild_boar.adb",
	...
	Properties =
	{
		TagList = "boar", -- CryMannequin tags for this entity, separated by '+'
...
function Boar_x:DoPainSounds(dead, hitType, isSilenced)
...
function Boar_x:DoFeedbackHit2DSounds(hit)
...
function Boar_x:EnableBlindFire(enable)
	if (enable) then
...
function Boar_x:CheckForMountedWeapons()
...
function Boar_x:FindValidMountedWeapon()
...
function Boar_x:OnUseMountedWeaponRequest(weaponId)
	self:PrepareForMountedWeaponUse(weaponId)
end

function Boar_x:PrepareForMountedWeaponUse(weaponId, pipeID)
	local weapon;
	if (weaponId) then
...
function Boar_x:ForceReleaseOfMountedGun()
	self:LeaveMountedWeapon();
end

function Boar_x:SetMachineGunInitialDirInRefPoint()
...
function Boar_x:UseMountedWeapon()
...
function Boar_x:LeaveMountedWeapon()
	if (self.AI.targetMountedWeapon) then
...
function Boar_x:CanShootMountedWeapon()
...
function Boar_x:CheckMountedWeaponTarget()
...
function Boar_x:MountedWeaponTargetChange()
	local targetType = AI.GetTargetType(self.id);
...
function Boar_x:IsFurthestAwayFromTargetInSquad()
...
function Boar_x:IsFurtherAwayFromTargetThanAveragePositionOfSquadScopeUsers(scopeName)
...
function Boar_x:IsClosestToTargetInSquad()
	-- Generate a table of the squad members
	local squadId = GameAI.GetSquadId(self.id)
	if (squadId == nil) then
...
function Boar_x:IsClosestToTargetInGroup()
	-- See who is the closest? (Start out with me.)
	local closestId = self.id
	local closestDist = 1000.0
...
function Boar_x:ShouldUseGrenade()
...
function Boar_x:RefreshCurrentDeadGroupMemberBodyPosition()
...
function Boar_x:SetDeadGroupMemberBodyPositionAsRefPoint()
	if (self.deadGroupMemberData == nil) then
...
function Boar_x:SetDeadGroupMemberDeathPositionAsRefPoint()
	if (self.deadGroupMemberData == nil) then
...
function Boar_x:GrabDeadGroupMemberDataFromGroupScriptTable()
	local groupId = AI.GetGroupOf(self.id)
